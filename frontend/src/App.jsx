      ...
      <div className="bg-white rounded-2xl shadow-2xl p-10 max-w-md w-full">
        ...
        <div className="mt-6">
          ...
          <button
            ...
            disabled={loading}
            className="mt-4 w-full flex items-center justify-center px-4 py-3 border border-gray-300 rounded-lg shadow-sm bg-white text-gray-700 hover:bg-gray-50 transition disabled:opacity-50"
          >
...
export default function EmailAutomationApp() {
  ...
  useEffect(() => {
    const handleOAuthCallback = async () => {
      ...
      if (code && state) {
        ...
        try {
          // First try Google login callback
          const loginResponse = await fetch(`${API_BASE}/api/auth/google/callback`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
          ...
          if (loginResponse.ok) {
            ...
            setShowAuth(false);
            return; // Stop here, don't try Gmail callback
          }
          
          // If login callback failed, try Gmail connection callback
          const gmailResponse = await fetch(`${API_BASE}/api/oauth/gmail/callback`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ code, state })
          });
          ...
          if (gmailResponse.ok) {
            ...
            const updatedUser = { ...currentUser, gmailConnected: true };
  ...
  if (loading || oauthProcessing) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-gradient-to-br from-blue-50 to-purple-50">
        ...
        <p className="text-gray-600">{oauthProcessing ? 'Completing sign in...' : 'Loading...'}</p>
      </div>
    );
  }
  ...
  if (!user) {
    if (showAuth) {
      return <AuthPage onLogin={(userData) => {
        login(userData);
        setShowAuth(false);
        // Check if Gmail is already connected from Google login
        if (userData.gmail_connected) {
          // Skip Gmail connect screen, go straight to dashboard
          return;
        }
  ...
  if (!user.gmail_connected && !user.gmailConnected) {
    if (showGmailConnect) {
      return <GmailConnect onConnect={() => {
        login({...user, gmailConnected: true, gmail_connected: true});
